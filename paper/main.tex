...
vehicular communications, yet training multimodal fusion models remains
computationally expensive in scarce-data settings. We present \textbf{mmBeamKD},
a framework combining parameter-efficient multimodal fusion with cross-modal
knowledge distillation for beam prediction on the DeepSense~6G Scenario-32
benchmark (1905 training samples, 64-beam classification).
\ours{} freezes pretrained ResNet backbones and trains only a lightweight
cross-attention fusion module (21.7\,M trainable parameters vs.\ 78.4\,M
baseline), achieving a $1.66\times$ inference speedup and a 29\% smaller
deployment checkpoint.

Modality ablation reveals that camera and GPS dominate in Scenario-32:
removing either collapses DBA by more than 37\%, while LiDAR and radar
contribute negligibly. Building on this finding, we apply cross-modal
knowledge distillation from a four-model heterogeneous teacher ensemble
to a camera+GPS-only student (\textsc{CamGPS-only-KD}), which achieves
DBA\,=\,0.8356 ($+0.028$ vs.\ baseline; 95\,\% CI: $[+0.017,\,+0.040]$;
$p < 0.0001$) at 19.9\,ms inference, matching the full three-model ensemble
while requiring only two sensors. A four-modality distilled student (\distill{})
achieves DBA\,=\,0.8285 ($+0.021$; $p=0.0002$). Per-beam analysis exposes a
systematic accuracy gap for high-index beams (DBA\,=\,0.694, beams 43--63),
linked to a training-to-test beam-distribution shift.
...
knowledge distillation, cross-modal distillation, parameter-efficient learning,
distribution shift
\end{IEEEkeywords}
...
that overfit when only a few thousand labeled samples are available, a constraint common to freshly deployed vehicular sites. Second, they rarely examine
...
(ii) a controlled sensor-attribution analysis, and (iii) a methodology including bootstrap
testing, distribution-shift analysis, and a beam-probing overhead metric, all of which transfer to other scenarios.

This paper presents \textbf{mmBeamKD}, which addresses all three gaps on
DeepSense~6G Scenario~32, a single urban intersection site with 1905 training,
635 validation, and 635 test sequences.
mmBeamKD comprises two stages: (i)~\ours{}, a parameter-efficient fusion backbone
that replaces the baseline's eight-layer GPT transformer with a compact two-layer
cross-attention module applied to frozen pretrained ResNet features, reducing
trainable parameters from 78.4~M to 21.7~M and cutting inference latency by
$1.66\times$; and (ii)~cross-modal knowledge distillation from a heterogeneous
four-model ensemble teacher, yielding both a four-modality student (\distill{})
and a camera+GPS-only student (\textsc{CamGPS-only-KD}) that matches the full
ensemble at 19.9~ms. Modality ablation reveals an unexpected finding: in
Scenario~32, \emph{camera and GPS are nearly sufficient for full performance},
while LiDAR and radar contribute negligibly; cross-modal KD exploits this finding directly.
...
lower overhead, which is a prerequisite for vehicular latency targets.
...
beamforming gain retained by the model is 82.9\% of the oracle gain, compared with 80.8\% for the baseline.
...
LiDAR and radar after training. The model does not need them from the outset.
...
more often lying on the boundary between adjacent beam groups, suggesting that
...
radar inputs (not merely zeroed at test time), achieves DBA\,=\,0.8064, which is statistically indistinguishable from the full model (0.8058). This control rules
...
The finding that a camera+GPS-only model achieves DBA\,=\,0.8064, equivalent to the full four-modality model, with direct deployment implications. LiDAR sensors
...
This paper presented \textbf{mmBeamKD}, a framework for sensing-aided mmWave beam
prediction combining parameter-efficient fusion with cross-modal knowledge
distillation, evaluated on DeepSense~6G Scenario~32.
The fusion backbone \ours{} freezes pretrained ResNet encoders and trains only a
compact cross-attention core with 21.7~M parameters, a 3.6$\times$ reduction over the baseline, while matching baseline DBA at $1.66\times$ lower latency
and a 29\,\% smaller checkpoint.
...
$p < 0.0001$) at three-fold inference overhead. Third, and most notably, a camera+GPS-only student distilled from the same four-modality
